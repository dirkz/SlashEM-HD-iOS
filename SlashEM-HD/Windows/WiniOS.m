...
winid ios_create_nhwindow(int type)
{
    ...
    switch (type) {
        case NHW_MESSAGE:
            ...
            break;
        case NHW_STATUS:
            sharedInstance.statusWindow = [[StatusWindow alloc] init];
            window = sharedInstance.statusWindow;
            break;
        case NHW_MAP:
            sharedInstance.mapWindow = [[MapWindow alloc] init];
            window = sharedInstance.mapWindow;
            break;
        case NHW_MENU:
            window = [[NhWindow alloc] initWithType:type];
            [sharedInstance.windows addObject:window];
            break;
        case NHW_TEXT:
            window = [[NhWindow alloc] initWithType:type];
            [sharedInstance.windows addObject:window];
...
void ios_destroy_nhwindow(winid wid)
{
    ...
    NSUInteger i = 0;
    for (id obj in sharedInstance.windows) {
        if (wid == (winid) obj) {
            break;
        }
        i++;
    }
    if (i < sharedInstance.windows.count) {
        [sharedInstance.windows removeObjectAtIndex:i];
    } else {
        LOG_WINIOS(1, @"unhandled destroy_nhwindow %@", wid);
    }
}
